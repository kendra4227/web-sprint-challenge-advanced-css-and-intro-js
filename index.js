...
function getArtistByIndex(arr, index) {
    let string = "";
    string = `The artist at index ${index} is ${arr[index].name}`;
    return string;
}
...
function get20s(arr) {
    ...
    for (i = 0; i < arr.length; i++) {
        num = parseInt(arr[i].years);
        if (num > 1900) {
            CenturyArtists.push(arr[i].name);
...
function lotsOfArt(arr) {
    ...
    let num1 = [];
    for (i = 0; i < arr.length; i++) {
        num1 = parseInt(arr[i].paintings)
        if (num1 > 100) {
            mostPaintings.push(arr[i].name);
        }
